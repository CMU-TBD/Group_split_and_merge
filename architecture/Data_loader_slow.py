...
class DataLoader(object):
	...
	def __init__(self):
		...
		for i in range(self.num_positive_data):
			...
			if i < self.num_train_data:
				train_img_seqs.append(self.candidate_img_seqs[idx, :, :, :, :])
				train_action_seqs.append(self.candidate_action_seqs[idx])
				train_location_seqs.append(self.candidate_location_seqs[idx, :])
			else:
		...
		self.test_img_seqs = np.array(self.test_img_seqs)
		self.test_action_seqs = np.array(self.test_action_seqs)
		self.test_location_seqs = np.array(self.test_location_seqs)

		self.split_img_seqs = []
		self.split_action_seqs = []
		self.split_location_seqs = []
		self.merge_img_seqs = []
		self.merge_action_seqs = []
		self.merge_location_seqs = []
		for i, action in enumerate(train_action_seqs):
			if action == -1:
				self.split_img_seqs.append(train_img_seqs[i])
				self.split_action_seqs.append(-1)
				self.split_location_seqs.append(train_location_seqs[i])
			elif action == 1:
				self.merge_img_seqs.append(train_img_seqs[i])
				self.merge_action_seqs.append(1)
				self.merge_location_seqs.append(train_location_seqs[i])
			else:
				print('Error in actions')
				return
		self.split_img_seqs = np.array(self.split_img_seqs)
		self.split_action_seqs = np.array(self.split_action_seqs)
		self.split_location_seqs = np.array(self.split_location_seqs)
		self.merge_img_seqs = np.array(self.merge_img_seqs)
		self.merge_action_seqs = np.array(self.merge_action_seqs)
		self.merge_location_seqs = np.array(self.merge_location_seqs)
	...
	def _add_negative_sequences(self, samples_needed):
	...
	def _resample_data(self):
		split_idxes = np.random.permutation(len(self.split_action_seqs))
		merge_idxes = np.random.permutation(len(self.merge_action_seqs))
		candidate_positive_img_seqs = []
		candidate_positive_action_seqs = []
		candidate_positive_location_seqs = []
		for i in range(self.sample_number):
			candidate_positive_img_seqs.append(self.split_img_seqs[split_idxes[i]])
			candidate_positive_action_seqs.append(self.split_action_seqs[split_idxes[i]])
			candidate_positive_location_seqs.append(
				self.split_location_seqs[split_idxes[i]])
			candidate_positive_img_seqs.append(self.merge_img_seqs[merge_idxes[i]])
			candidate_positive_action_seqs.append(self.merge_action_seqs[merge_idxes[i]])
			candidate_positive_location_seqs.append(
				self.merge_location_seqs[merge_idxes[i]])
